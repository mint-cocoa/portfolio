...
`portfolio.mintcocoa.cc`는 `mint-cocoa.github.io` 루트 허브를 C++ `RuntimeWeb` 컨테이너의 `/`에서 서빙하고, 같은 컨테이너의 `/portfolio/` 아래에 상세 문서 산출물을 함께 싣는 홈랩 미러 경로입니다.

- Canonical docs: `https://mint-cocoa.github.io/portfolio/`
- Homelab mirror: `https://portfolio.mintcocoa.cc/`
- Homelab docs mirror: `https://portfolio.mintcocoa.cc/portfolio/`
...
컨테이너는 Docker build 중 `mint-cocoa/mint-cocoa.github.io`를 빌드해 `/usr/share/portfolio` 루트에 두고, 이 저장소의 `docs/` 산출물은 `/usr/share/portfolio/portfolio` 아래에 복사합니다.
